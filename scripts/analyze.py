...
def chunk_commits(commits: list[Commit], strategy: str,
    ...
    return _affinity_groups(commits, coupling=coupling, communities=communities)
...
def cluster(chunks: list[list[Commit]], coupling: Optional[dict] = None,
            communities: Optional[dict] = None, cap: int = 8) -> list[list[Commit]]:
    ...
    co-change coupling, Leiden community match. Deterministic.
    """
    m = len(chunks)
    parent = list(range(m))
...
def analyze(repo: str, history_scope: str = "full", code_scope: str = "repo") -> dict:
    ...
    chunks = chunk_commits(commits, strategy, coupling=coupling, communities=communities)
    clusters = cluster(chunks, coupling=coupling, communities=communities)
